...
При регистрации и обновлении профиля email должен быть полным адресом, например `user@gmail.com` или `user@example.com`.
Для `PUT /api/me` можно передать поле `password`; если поле пустое или не передано, старый пароль остается без изменений.
...
Pending-заявок может быть несколько; пользователь принимает или отклоняет конкретную заявку по ее `id`.
